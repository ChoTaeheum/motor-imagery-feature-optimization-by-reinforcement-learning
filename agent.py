...
class Agent():
	...
	def act(self, s):
		...
		for i in range(len(rand_agents)):
			...
			w = int(rand_number[i] * len(possible_actions))    # 남아있는 피쳐에서 랜덤으로 아무거나 선택
			a[agent] = possible_actions[w]
		return a    # 천개의 agent가 각 한개씩의 action을 수행
